...
void main() {
  test('theme defaults match the chip spec', () {
    ...
    expect(
      theme.elevation(elevated: true, dragged: false),
      M3EElevation.level1,
    );
    expect(theme.elevation(elevated: true, dragged: true), M3EElevation.level4);
  });

  testWidgets('chips shrink-wrap so a wrap can place them side by side', (
    tester,
  ) async {
  ...
  testWidgets('text chip is 32 tall with 16 padding', (tester) async {
  ...
  testWidgets('input chip with both actions is at least 88 wide', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
    ...
    expect(tester.getSize(find.byType(M3EChip)).height, 32);
    expect(
      tester.getSize(find.byType(M3EChip)).width,
  ...
  testWidgets('filter chip uses checkbox semantics', (tester) async {
    await tester.pumpWidget(
      _host(
        M3EChip(
          label: 'News',
  ...
  testWidgets('space toggles a filter chip', (tester) async {
    var selected = false;
    await tester.pumpWidget(
      _host(
        M3EChip(
  ...
  testWidgets('arrows move focus inside a chip group', (tester) async {
    await tester.pumpWidget(
      _host(
        M3EChipGroup(
          child: Column(
  ...
  testWidgets('arrows inside a list do not scroll', (tester) async {
    await tester.pumpWidget(
      M3EMaterialApp(
        data: M3EThemeData.light(seedColor: const Color(0xFF6750A4)),
        home: Scaffold(
  ...
  testWidgets('backspace removes a focused input chip', (tester) async {
    var deleted = 0;
    await tester.pumpWidget(
      _host(
        M3EChipGroup(
  ...
  testWidgets('dragging raises the chip to level 4', (tester) async {
    final scheme = M3EThemeData.light(seedColor: const Color(0xFF6750A4))
        .colorScheme;
