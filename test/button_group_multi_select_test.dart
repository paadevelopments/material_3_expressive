...
void main() {
  _registerMultiSelectTests();
}

void _registerMultiSelectTests() {
  testWidgets(
    'multiSelect toggles multiple actions independently',
    _multiSelectTogglesMultipleActionsIndependently,
  );
  testWidgets(
    'multiSelect selectionRequired blocks clearing the last item',
    _multiSelectSelectionRequiredBlocksClearingTheLastItem,
  );
}

Future<void> _multiSelectTogglesMultipleActionsIndependently(
  WidgetTester tester,
) async {
  var selected = <int>{0};
...
Future<void> _multiSelectSelectionRequiredBlocksClearingTheLastItem(
  WidgetTester tester,
) async {
  var selected = <int>{1};
