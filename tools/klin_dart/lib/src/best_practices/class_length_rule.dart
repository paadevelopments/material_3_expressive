...
/// Warns when a class declaration exceeds the configured line count.
class ClassLengthRule extends DartLintRule {
  ...
  /// Reads `max_lines` and `stateful_widget_max_lines` from [config].
  ClassLengthRule({Map<String, Object?>? config})
    : maxLines =
          int.tryParse(config?['max_lines']?.toString() ?? '') ??
          _defaultMaxLines,
      statefulWidgetMaxLines =
          int.tryParse(
            config?['stateful_widget_max_lines']?.toString() ?? '',
          ) ??
          _defaultStatefulWidgetMaxLines,
      super(
        code: const LintCode(
          name: 'class_length',
          problemMessage:
              'Class is too long ({0} lines). Maximum allowed is {1} lines.',
          correctionMessage: 'Consider breaking this class into smaller, more focused classes.',
